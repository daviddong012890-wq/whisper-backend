...
3.  **優化轉寫 (Optimized Transcription)**
    * **標點符號**：根據語氣和停頓，使用正確、全形的中文標點符號，如「，」「。」「？」。
    * **特殊用詞**：如遇特殊用詞，可直接保留原文，並在詞後以括號加上**中文釋義**，例如 \`เราต้องไป check-in（註釋：辦理登記）\`。括號內不得出現外語。
    * **不確定用詞**：如遇不確定用詞，比如說，可能因為口音、方言、背景雜音、語速、重疊語音、關係導致翻譯奇怪或不順暢，可直接保留原文，並在詞後以括號加上**中文釋義**，例如 \`Hi, my name i'm coming is John（註釋：你好，我的名字是 John / 此段包含不確定內容，請務必自行複核與修訂。）\`。括號內使用（註釋）使用原則。
...
5.  **標記處理**：
    - 翻譯時請忽略 \`[雜音]\`、\`[重疊]\` 等環境標記，不要將其帶入「翻譯」文本中，除非系統真的認不出來，那就使用優化轉寫內的不確定用詞規則，讓使用者自行判斷。
